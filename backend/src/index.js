                      ...
                      {(() => {
                        ...
                        if (waveformData && waveformData.amplitudes) {
                          ...
                          for (let i = -10; i < numBars; i++) {
                            ...
                            // Skip if before song start or after duration
                            if (timeAtBar < 0 || timeAtBar > waveformData.duration) continue;
                            
                            // Get the sample index for this time position
                            const sampleIndex = Math.floor(timeAtBar * samplesPerSecond);
                            
                            // Get amplitude (0-1) from waveform data
                            let amplitude = 0;
                            if (sampleIndex >= 0 && sampleIndex < waveformData.amplitudes.length) {
                              amplitude = waveformData.amplitudes[sampleIndex];
                            }
                          ...
                          // Fallback: Show flat line or loading indicator if no waveform data
                          for (let i = -10; i < numBars; i++) {
                            const barX = (i * totalBarWidth);
                            const timeAtBar = currentTime + (barX - centerX) / zoom;
